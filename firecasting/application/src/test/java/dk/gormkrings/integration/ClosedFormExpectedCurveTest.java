...
class ClosedFormExpectedCurveTest {

    @Test
    void fixedNominalReturn_andFixedInflation_withNoCashflows_matchesClosedFormCurve_yearly() {
        // Arrange
        final int years = 30;
        final double initialBalance = 100_000.0;

        // Annual nominal return, expressed as a fraction: 0.07 => 7%.
        final double annualNominalReturn = 0.07;

        // Annual inflation factor: 1.02 => 2% inflation.
        final double annualInflationFactor = 1.02;
        ...
        // Ensure deterministic cadence: apply returns at MONTH_END using dt=1/12.
        ...
        // Phase 1: one-time initial balance only.
        ...
        // Phase 2: passive growth for N years, no additional deposits/withdrawals.
        ...
        for (int year = 0; year <= years; year++) {
            ...
            double expectedCapital = initialBalance * Math.pow(1.0 + annualNominalReturn, year);
            double expectedInflationIndex = Math.pow(annualInflationFactor, year);
            double expectedRealCapital = expectedCapital / expectedInflationIndex;

            assertClose(expectedCapital, state.getCapital(), "capital", year);
    ...
    private static void assertClose(double expected, double actual, String field, int year) {
        // Allow tiny accumulated floating-point error from repeated per-step compounding.
        double absTol = 1e-3;
        double relTol = 1e-8;
